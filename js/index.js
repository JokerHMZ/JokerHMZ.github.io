var HyFn = (function(window, undefined){
	...
	var manifest = [
		...
		{src:'../image../s/rect4.png', id:'rect4'},
		...
		{src:'../img/prizeExplainText1.png', id:''},
		{src:'../img/prizeExplainText2.png', id:''},
		...
		{src:'../img/statementWord1.png', id:''}
	];

	var initIMG1 = {w:10, maxH:185, minH:10, maxT:500, count:64};//275
	...
	var load = function(){
		...
		function handleComplete() {
			...
			_utaq.push(['trackEvent','pv-1']);//ut,page1
			var one = true;
			if(!$('.load_btn').hasClass('box2_b')) {
				$('.load_btn').on(evStart, function(){
					if(one) {
						one = !one;
						!!media ? stm_clicki('send', 'event', '首页', 'click', '聆听') : null;
						...
						setTimeout(function(){
							init();
							_utaq.push(['trackEvent','pv-2']);//ut,page2
						}, 500);
					}
				});
			}
	...
	var init = function() {
		...
		!!media ? stm_clicki('send', 'pageview', {'page': '/tlsh5/1', 'title': 'ani'}) : null;
	}
		...
		createjs.Ticker.addEventListener('tick', function(e){
			...
			if(SKIP===false) {
				switch (ticks) {
					...
					case 500:
						$('.box1').fadeIn(500);
						_utaq.push(['trackEvent','pv-3']);//ut,page3
						!!media ? stm_clicki('send', 'pageview', {'page': '/tlsh5/2', 'title': 'finish'}):null;
						break;
				...
				container.removeChild(contain1,contain2,contain4);
				lists1 = lists2 = lists2_o = lists4 = lists4_o = null;
				container.x = -2946;
				if(!$('.box2').is(':visible')) {
					$('.box2').fadeIn(500);
					_utaq.push(['trackEvent','pv-4']);//ut,page4
				}
			}
			stage.update();
	...
	return {
		init: function(){
			...
			$('.box1').on(evStart, function(){
				...
				$('.box2').fadeIn(700);
				_utaq.push(['trackEvent','pv-4']);//ut,page4
				!!media ? stm_clicki('send', 'pageview', {'page': '/tlsh5/3', 'title': 'tvc'}):null;
			});
